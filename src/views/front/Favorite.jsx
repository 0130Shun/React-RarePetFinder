...
const Favorite = () => {
  ...
  // 我把「取消收藏後要做什麼」這件事，交給頁面自己決定，要取消收藏+卡片要消失 => onRemove: (storeId)
  const { toggleFavorite } = useFavorite(user, favoritesMap, setFavoritesMap, {
    onRemove: (storeId) => {
      setItems((prev) => prev.filter((item) => item.id !== storeId));
      setTotalCount((prev) => prev - 1);
    },
  });

  // 從storeService 載入api工具 getAllStores()：第一次載入抓資料「全部店家」
  useEffect(() => {
    let mounted = true;
    ...
    async function load() {
      ...
      try {
        ...
        const data = await getFavoriteStores(user.id); // 所有店家抽換成user自己的已點選喜愛店家
